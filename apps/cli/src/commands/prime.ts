...
<commands>
<session>
Sessions group searches so you can come back later and reference them by name.

flt session start ["name"]     Start a named session (closes any active one)
flt session close              Close the active session
flt session list               List all sessions (active + closed)
flt session rename "new name"  Rename active session (or --id s1 for a specific one)

Auto-behavior:
- Searching without an active session auto-starts one (named after the route).
- Running \`flt takeout\` auto-closes the active session. Use \`--keep-session\` to prevent this.

To revisit a past session's searches, use \`flt session list\` to find its refs.
</session>
...
0. Session:
   - If no active session, one will auto-start on first search. Or run: \`flt session start "Trip name"\`.
   - Start every response with: **session: <name> (<id>)** — or **session: none** if no session is active.
   - When a session auto-starts or auto-closes, acknowledge it briefly.
...
6. End of session:
   ...
   - Takeout auto-closes the session. Mention this to the user.
   - To revisit a closed session later, use \`flt session list\` to see past sessions and their search refs.
