...
let itineraryPanel: ReturnType<typeof ItineraryPanel> | undefined

function flightsToOffers(flights: Flight[], googleUrl: string): Offer[] {
  return flights.map((f, i) => ({ ...f, id: `O${i + 1}`, url: googleUrl }))
}
...
async function handleSingleSearch(params: SearchParams) {
  ...
  try {
    ...
    if (result) {
      offers = flightsToOffers(result.flights, result.google_flights_url)
